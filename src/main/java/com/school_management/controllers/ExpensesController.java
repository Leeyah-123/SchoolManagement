...
import com.school_management.dao.ExpensesDB;
...
public class ExpensesController implements Initializable {
    ...
    @FXML
    private MFXLegacyTableView<Expense> tableView;
    @FXML
    private TableColumn<Expense, String> columnRecordID, columnItem, columnDescription, columnCost, columnDate;
    @FXML
    private Label txtBalance;

    ObservableList<Expense> records = FXCollections.observableArrayList();
    ...
    private void addRecord() {
        ...
        MFXTextField item = (MFXTextField) centerPane.getChildren().get(0);
        MFXTextField description = (MFXTextField) centerPane.getChildren().get(1);
        MFXTextField cost = (MFXTextField) centerPane.getChildren().get(2);

        AnchorPane bottomPane = (AnchorPane) dialog.getChildren().get(2);
        MFXButton saveBtn = (MFXButton) bottomPane.getChildren().get(0);

        saveBtn.setOnAction(event -> {
            if (item.getText().equals("") || description.getText().equals("") || cost.getText().equals("")) {
                Alerts.AlertError("Error", "Please fill out each field");
                return;
            }

            if (!Validators.isValidName(item.getText())) {
                Alerts.AlertError("Error", "Invalid Item Name");
                return;
            }

            if (description.getText().trim().matches("[a-zA-Z]+")) {
                Alerts.AlertError("Error", "Invalid description");
                return;
            }

            if (!cost.getText().matches("[0-9]+")) {
                Alerts.AlertError("Error", "Invalid cost");
                return;
            }

            if (AccountsDB.currentBalance() < Integer.parseInt(cost.getText())) {
                Alerts.AlertError("Error", "Insufficient account balance");
                return;
            }

            if (ExpensesDB.addRecord(item.getText(), description.getText(), cost.getText()) != 1) {
                Alerts.AlertError("Error", "Something went wrong");
                return;
            }

            if (AccountsDB.addRecord("debit", description.getText(), cost.getText(), AccountsDB.currentBalance() - Integer.parseInt(cost.getText())) != 1) {
                Alerts.AlertError("Error", "Something went wrong");
                return;
            }
    ...
    private MFXGenericDialog load_add_record_dialog() {
        ...
        MFXTextField item_purchased = new MFXTextField();
        item_purchased.setFloatingText("Item Purchased");
        item_purchased.setFloatMode(FloatMode.BORDER);
        item_purchased.setLayoutX(81);
        item_purchased.setLayoutY(14);
        ...
        centerPane.getChildren().addAll(item_purchased, description, amount);
    ...
    private void populateTable(ResultSet rs) throws SQLException {
        while (true) {
            ...
            record.setRecordID(rs.getInt(Expense.EXPENSE_RECORD_ID));
            record.setItemPurchased(rs.getString(Expense.ITEM_PURCHASED));
            record.setCost(rs.getString(Expense.EXPENSE_COST));
            record.setDescription(rs.getString(Expense.EXPENSE_DESCRIPTION));
            record.setDate(rs.getString(Expense.EXPENSE_DATE));
            records.add(record);
        }
    ...
    private void setTableView() {
        ...
        columnItem.setCellValueFactory(new PropertyValueFactory<>("itemPurchased"));
    ...
    private void refresh() {
        records.clear();
        try {
            populateTable(Objects.requireNonNull(ExpensesDB.getRecords()));
